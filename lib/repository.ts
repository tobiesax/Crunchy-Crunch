...
export async function getProducts(): Promise<Product[]> {
  ...
  const { data, error } = await admin.from("products").select("external_id,name,description,price,image,active,category,weight").eq("merchant_id", id).eq("active", true).order("created_at");
  if (error) throw new Error(error.message);
  return (data ?? []).map((row) => ({ id: row.external_id, merchantId: id, name: row.name, description: row.description, price: money(row.price), image: row.image, active: row.active, category: row.category ?? "Cookies", weight: row.weight ?? "" }));
}
...
export async function getDashboardData(): Promise<StoreData> {
  ...
  const [productResult, customerResult, orderResult, itemResult, eventResult] = await Promise.all([
    supabase.from("products").select("external_id,name,description,price,image,active,category,weight").eq("merchant_id", id),
  ...
  return {
    products: (productResult.data ?? []).map((row) => ({ id: row.external_id, merchantId: id, name: row.name, description: row.description, price: money(row.price), image: row.image, active: row.active, category: row.category ?? "Cookies", weight: row.weight ?? "" })),
